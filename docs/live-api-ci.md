...
- The live tests use `#[tokio::test(flavor = "multi_thread", worker_threads = 1)]` so the guard's Drop can call `tokio::task::block_in_place` + `Handle::current().block_on` for synchronous cleanup. The CI live job invokes `cargo test --test live_smoke --no-default-features --features rustls-ring-tls,indicatif --locked -- --nocapture --test-threads=1`; the `--test-threads=1` flag also keeps the cleanup tasks well below IA's per-user rate limit.
