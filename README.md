...
`publish_item` and `upsert_item` return as soon as every file uploads. They do not wait for the item to appear in the public catalog, because archive.org ingests new items asynchronously and that projection can take many minutes. By default `PublishOutcome::item` is therefore `None` and `PublishOutcome::projection_confirmed()` is `false`, even though the upload succeeded and the bytes landed. If you need the catalogued item back, call `PublishRequest::wait_for_visibility(duration)` to poll for projection up to a bound. That wait is best-effort: on timeout it still returns `Ok` with `item` left as `None`, so a successful upload is never turned into an error or a hang.

A new item is always created with its header metadata (title, collection, creator, and similar simple fields). Non-header metadata (nested objects or other complex values) cannot be applied until the item is catalogued, so on a brand-new item it is deferred by default and written only when you set `wait_for_visibility`. Updating an existing item applies all metadata immediately, since it is already catalogued.
...
```toml
internetarchive-rs = { version = "0.5", features = ["indicatif"] }
```
